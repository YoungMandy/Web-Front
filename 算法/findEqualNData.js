...
function hasAnswer (data, n, sum) {
  ...
  if (n > 0) {
    for (let i = 0; i < data.length; i++) {
      const temp = data.slice(i + 1, data.length);
      return hasAnswer(temp, n - 1, sum - data[i]) || hasAnswer(temp, n, sum);
     }
      
  }
}

console.log(hasAnswer([-1, 1, 2,3,4,6],1,8));